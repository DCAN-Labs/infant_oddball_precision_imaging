...
def task_code_ION(sub, acq, perm):
    ...
    #shuffle design matrix to generate noise distribution
    columns_to_scramble = ['Noise', 'Noise_derivative']

    # Scramble the specified columns
    for column in columns_to_scramble:
        initial_design_matrix[column] = np.random.permutation(initial_design_matrix[column].values)
    ...
    for index, (contrast_id, contrast_val) in enumerate(contrasts.items()):
        ...
        # save cifti
        nb.cifti2.save(cifti_data_dscalar3, f"/tmp/sub-{sub}/acq-{acq}/noise/sub-{sub}_acq-{acq}_contrast_{contrast_id}_zscored_noise_perm{perm}.dscalar.nii")
...
if __name__ == "__main__":
    ...
    parser.add_argument("perm", help="permutation number for noise distribution")
