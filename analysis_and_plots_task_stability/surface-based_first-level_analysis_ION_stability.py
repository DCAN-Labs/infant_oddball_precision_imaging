...
def task_code_ION(sub, acq, perm):
    ...
    oname = os.path.join(defined_path2, f'sub-{sub}_ses-{ses}_acq-{acq}_mask_runs.txt')
    ...
    fname2 = os.path.join(defined_path2,
                          f'sub-{sub}_ses-{ses}_task-{task}_acq-{acq}_bold_shuffled_timeseries_scrubbed.dtseries.nii')
    ...
    for index, (contrast_id, contrast_val) in enumerate(contrasts.items()):
        ...
        # save cifti
        nb.cifti2.save(cifti_data_dscalar3, f"/mypath/oddball_task/stability/half1/ION{sub}/sub-{sub}_acq-{acq}_contrast_{contrast_id}_zscored_perm{perm}_h1.dscalar.nii")
