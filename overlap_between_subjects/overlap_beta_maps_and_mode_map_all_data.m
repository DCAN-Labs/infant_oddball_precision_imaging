...
filelist=dir([output '/*at25*.dscalar.nii']);
%%
% read in all betas and restrict to data that passed threshold (positive)

for n=1:size(filelist)
    b=cifti_read([output '/' filelist(n,1).name]);
    data=b.cdata;
    datahigh=find(data>0);
...
% read in eta file for teh purpose of having network labels in matlab
...
writematrix(netwpctpos, [output '/values_networks_and_significant_late_betas_all_data.csv'])
%% try out spider plot
