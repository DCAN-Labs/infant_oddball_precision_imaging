...
def task_code_ION(sub, acq):
    ...
    if "7T" in acq:
        ...
        ename = os.path.join(event_path, f"ION{sub}_event_file_habi.csv")
    ...
    figname=f'/mypath/oddball_task/habituation/sub-{sub}_acq-{acq}_initial_design_matrix_habi'
    plt.savefig(figname)
    ...
    figname2=f'/mypath/oddball_task/habituation/sub-{sub}_acq-{acq}_censored_design_matrix_habi'
    ...
    #same analysis fro middle or late oddballs
    contrasts = {
        'oddball': (
            basic_contrasts['NoiseE']+
            basic_contrasts['NoiseE_derivative'])
    }
    ...
    for index, (contrast_id, contrast_val) in enumerate(contrasts.items()):
        ...
        # save cifti
        nb.cifti2.save(cifti_data_dscalar3, f"/mypath/oddball_task/habituation/sub-{sub}_acq-{acq}_contrast_{contrast_id}_zscored_habi_early.dscalar.nii")
